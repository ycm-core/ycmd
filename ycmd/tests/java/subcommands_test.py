...
class SubcommandsTest( TestCase ):
  ...
  def test_Subcommands_GoToCallers( self, app ):
    for test in [
      { 'request': { 'line': 6, 'col': 17, 'filepath': TEST_JAVA },
        'response': [ has_entries( { 'line_num': 12, 'column_num': 10, 'filepath': TEST_JAVA } ) ],
        'description': 'GoToCallers on a function call.' },
      { 'request': { 'line': 20, 'col': 16, 'filepath': TEST_JAVA },
        'response': [ has_entries( { 'line_num': 15, 'column_num': 41, 'filepath': TEST_JAVA } ) ],
        'description': 'GoToCallers on a class name produces constructor calls' },
    ]:
      with self.subTest( test = test ):
        RunGoToTest( app,
                     test[ 'description' ],
  ...
  def test_Subcommands_GoToCallees( self, app ):
    for test in [
      { 'request': { 'line': 26, 'col': 22, 'filepath': TESTLAUNCHER_JAVA },
        'response': [
          has_entries( { 'filepath': TESTLAUNCHER_JAVA, 'line_num': 27, 'column_num': 22 } ),
          has_entries( { 'filepath': TESTLAUNCHER_JAVA, 'line_num': 27, 'column_num': 22 } ),
          has_entries( { 'filepath': TESTLAUNCHER_JAVA, 'line_num': 28, 'column_num':  5 } ),
          has_entries( { 'filepath': TESTLAUNCHER_JAVA, 'line_num': 28, 'column_num':  5 } ),
          has_entries( { 'filepath': TESTLAUNCHER_JAVA, 'line_num': 28, 'column_num': 12 } ),
          has_entries( { 'filepath': TESTLAUNCHER_JAVA, 'line_num': 37, 'column_num':  5 } ),
        ],
