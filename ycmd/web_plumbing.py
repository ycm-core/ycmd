...
class Request:
  def __init__( self, env : str ) -> str:
    ...
    self.content_length = env.get( 'CONTENT_LENGTH' ) or 0
    self.body = env[ 'wsgi.input' ].read( int( self.content_length ) )
...
class AppProducer:
  ...
  def _ErrorHandler( self, http_error : HTTPError, response : Response ):
  ...
  def __call__( self, environ, start_response ) -> List[ bytes ]:
    ...
    try:
      ...
      if request.method == 'GET':
        out = self._get_routes[ request.path ]( request, response )
      else:
        out = self._post_routes[ request.path ]( request, response )
    except HTTPError as e:
      status = str( e.status ) + ' ' + http.client.responses[ e.status ]
      response = Response()
      ...
      status = '500 ' + http.client.responses[ 500 ]
      response = Response()
      out = self._ErrorHandler( e, response )
