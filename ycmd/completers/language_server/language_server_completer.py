...
class LanguageServerCompleter( Completer ):
  ...
  def GetSubcommandsMap( self ):
    ...
    if 'callHierarchyProvider' in self._server_capabilities:
      commands[ 'GetOutgoingCalls' ] = (
        lambda self, request_data, args:
            self.CallHierarchy( request_data, [ 'outgoing' ] )
      )
      commands[ 'GetIncomingCalls' ] = (
        lambda self, request_data, args:
  ...
  def CallHierarchy( self, request_data, args ):
    ...
    assert len( preparation_item ) == 1 # I have no idea what else to do.
